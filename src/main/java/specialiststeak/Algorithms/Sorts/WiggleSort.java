...
import static specialiststeak.Algorithms.Utils.QuickSelect.select;
 ...
 * Also have a look at: <a href="https://cs.stackexchange.com/questions/125372/how-to-wiggle-sort-an-array-in-linear-time-complexity?noredirect=1&lq=1">...</a>
    ...
    private <T extends Comparable<T>> T[] wiggleSort(T[] sortThis) {
        ...
        if (sortThis.length % 2 == 1 && numMedians == ceil(sortThis.length / 2.0)) {
            ...
            if (!(0 == smallestValue.compareTo(median))) {
                throw new IllegalArgumentException("For odd Arrays if the median appears ceil(n/2) times, " + "the median has to be the smallest values in the array.");
            }
        }
        if (numMedians > ceil(sortThis.length / 2.0)) {
            System.out.println("ERROR: No more than half the number of values may be the same.");
            System.out.println("Returning an unsorted array.");
            return sortThis;
        }
